...
public class MessageCreateor {
	...
	public static byte[] createRequestMessage(String fileName, int opcodeInt) {
		...
		System.arraycopy(fileNameByte, 0, REQUEST, opcode.length,
				fileNameByte.length);// check
	...
	public byte[] increment(byte[] valuea) {
		...
		int value = valuea[1] & 0xFF | (valuea[0] & 0xFF) << 8;
		value = value + 1;
	...
	public int byteToInt(byte[] value) {
		return value[1] & 0xFF | (value[0] & 0xFF) << 8;
	}
